...
//          新規モード
            ...
            //デフォルト表示グループ
            String dfGpSidStr = schBiz.getDispDefaultGroupSidStr(con, sessionUsrSid);
            ...
            dspGpSidStr = schBiz.getEnableSelectGroup(
...
//----------修正モード------------------------------------------------------//
        ...
        if (svReserves != null) {
            for (int i = 0; i < svReserves.length; i++) {
                ...
                int schPublic = NullDefault.getInt(
                        paramMdl.getSch040Public(), GSConstSchedule.DSP_ALL);
                if (schPublic == GSConstSchedule.DSP_NOT_PUBLIC
                        || schPublic == GSConstSchedule.DSP_YOTEIARI) {
                    schPublic = GSConstReserve.PUBLIC_KBN_PLANS;
                } else if (schPublic == GSConstSchedule.DSP_BELONG_GROUP) {
                    schPublic = GSConstReserve.PUBLIC_KBN_GROUP;
                }
                yrkParam.setRsyPublic(schPublic);
                    ...
                    for (int i = 0; i < svUsers.length; i++) {
                        ...
                        if (paramMdl.getSch040AttendKbn().equals(
                            ...
                            if (paramMdl.getSch040EditMailSendKbn()
                                ...
                                //出欠確認依頼メール
                                schBiz.sendAttendSmail(con__, cntCon__, scdMdl, appRootPath,
                            ...
                            schBiz.sendPlgSmail(con__, cntCon__, scdMdl,
   ...
   public String delTargetSet(Sch040ParamModel paramMdl,
       ...
       if (paramMdl.getSch040TimeKbn().equals(String.valueOf(GSConstSchedule.TIME_EXIST))) {
           ...
           usrNameList.append("\n");
           usrNameList.append(paramMdl.getSch040FrYear());
           usrNameList.append(gsMsg.getMessage("cmn.year2"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040FrMonth(), "00"));
           usrNameList.append(gsMsg.getMessage("cmn.month"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040FrDay(), "00"));
           usrNameList.append(gsMsg.getMessage("tcd.working.day"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040FrHour(), "00"));
           usrNameList.append(gsMsg.getMessage("cmn.hour"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040FrMin(), "00"));
           usrNameList.append(gsMsg.getMessage("cmn.minute"));
           usrNameList.append("~\n");

           usrNameList.append(paramMdl.getSch040ToYear());
           usrNameList.append(gsMsg.getMessage("cmn.year2"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040ToMonth(), "00"));
           usrNameList.append(gsMsg.getMessage("cmn.month"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040ToDay(), "00"));
           usrNameList.append(gsMsg.getMessage("tcd.working.day"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040ToHour(), "00"));
           usrNameList.append(gsMsg.getMessage("cmn.hour"));
           usrNameList.append(StringUtil.toDecFormat(paramMdl.getSch040ToMin(), "00"));
           usrNameList.append(gsMsg.getMessage("cmn.minute"));
       }
       //削除対象のセット
       usrNameList.append("\n\n");
