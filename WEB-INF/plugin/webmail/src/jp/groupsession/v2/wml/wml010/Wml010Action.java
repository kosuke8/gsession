...
public class Wml010Action extends AbstractWebmailAction {
    ...
    throws Exception {
        ...
        //メール送信テンポラリディレクトリを削除(前回分がある場合のみ)
        Wml010Biz biz = new Wml010Biz();
        String tempDir = biz.getSendTempDir(getTempPath(req), reqMdl, form.getWml010tempDirId());
        biz.deleteSendTempDir(tempDir);
    ...
    private ActionForward __doSendMail(ActionMapping map,
        ...
        try {
            ...
            try {
                ...
                String tempDir = biz.getSendTempDir(getTempPath(req), reqMdl,
                        paramMdl.getWml010tempDirId());
...
//                        }
                    ...
                    sendData.setSendPlanDate(draftSendData.getSendPlanDate());
                    draft = true;
                    timeSent = true;
            ...
            if (message == null) {
                ...
                try {
                    sendData = biz.saveDraftMail(con, paramMdl, getGsContext(),
                            getSessionUserSid(req), tempDir, reqMdl);
                    con.commit();
                    commit = true;
    ...
    private ActionForward __doSendFileUpload(ActionMapping map,
        ...
        try {
            ...
            String tempDirId = form.getWml010tempDirId();

            String tempDirPath = biz.getSendTempDir(getTempPath(req), reqMdl, tempDirId);
    ...
    //                                + Wml010Biz._escapeText(tempFile.getFileName()) + "\",");
                        ...
                        //テンポラリディレクトリへファイルを保存
                        String saveFileName = biz.uploadTempFile(tempFile, tempDirPath);
    ...
    private ActionForward __doSendFileDownload(ActionMapping map,
        ...
        String tempDirPath = biz.getSendTempDir(getTempPath(req), reqMdl,
                                                form.getWml010tempDirId());
    ...
    private ActionForward __doSendFileDelete(ActionMapping map,
        ...
        //テンポラリディレクトリID取得
        String tempDir = biz.getSendTempDir(getTempPath(req),
                                            getRequestModel(req),
                                            form.getWml010tempDirId());
        if (!tempDir.isEmpty()) {
