...
public class CirCommonBiz {
    ...
    public List<CirAccountModel> getAccountList(Connection con,
        ...
        = userBiz.getUserList(con, newUserSid);
        ...
        Map<Integer, CirAccountModel> accUsrMap = new HashMap<Integer, CirAccountModel>();
        for (CirAccountModel mdl : usrAccList) {
            accUsrMap.put(mdl.getUsrSid(), mdl);
        }
        for (CmnUsrmInfModel usrMdl : usList) {
            CirAccountModel mdl = accUsrMap.get(usrMdl.getUsrSid());
            if (mdl != null) {
                ret.add(mdl);
            }
        }
