  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="td_type1" align="left" valign="top">
        ...
        <% if (sdFlg) { %><span class="text_deluser_enq"><% } else if (ukoFlg == 1) {%><span class="mukouser"><% }%>
        <% if (!enq330anony) { %><a href="javaScript:void(0);" onClick="openUserInfoWindow(<bean:write name="ansData" property="userSid" />);" class="answer"><% } %>
        <bean:write name="ansData" property="user" />
        <% if (!enq330anony) { %></a><% } %>
        <% if (sdFlg | ukoFlg == 1) { %></span><% } %>
      </td>
      <!-- 回答日 -->
      <td class="td_type1" align="left" valign="top">
