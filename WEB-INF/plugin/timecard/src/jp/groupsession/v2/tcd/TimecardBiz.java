...
public class TimecardBiz {
    ...
    throws SQLException {
        ...
        for (TcdTcdataModel tcdMdl : tcdList) {
            ...
            if (!isOver24HoursOrTenAM(tcdMdl.getTcdDate(), inTime)) {
                ret = tcdMdl;
            }
        }
        if (ret == null) {
    ...
    /**
     * <br>[機  能] 始業から24時間、または翌日の午前10時を超えているか判定する
     * <br>[解  説]
     * <br>[備  考]
     * @param frDate 始業日
     * @param fr 始業時間
     * @return boolean true:24時間、または10時を超えている false:24時間、または10時を超えていない
     */
    public boolean isOver24HoursOrTenAM(UDate frDate, Time fr) {
        ...
        if (sysDate.compareDateYMD(tcDate) == UDate.SMALL) {
            if (sysDate.getIntHour() >= GSConstTimecard.TOMORROW_TEN_AM) {
                return true;
