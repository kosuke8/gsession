...
public class Rng030Biz {
    ...
    throws IOException, IOToolsException, SQLException, TempFileException {
        ...
        switch (apprMode) {
            ...
            case RngConst.RNG_APPRMODE_APPL :
                ...
                //稟議の状態が「却下」以外、または「完了」なら差し戻し以外の状態なので再申請できない
                if (model.getRngStatus() != RngConst.RNG_STATUS_REJECT
                        || model.getRngCompflg() == RngConst.RNG_COMPFLG_COMPLETE) {
                    applicateFlg = false;
                }

                //再申請ができない状態の場合
                if (!applicateFlg) {
                    paramMdl.setRngApprMode(RngConst.RNG_APPRMODE_APPR);
                    int rncStatus = model.getRncStatus();
                    ringiModeSet(paramMdl, rngSid, userSid, model, cmd, channelDao, rncStatus, con);
                    break;
                }
     ...
     * @param cmd コマンド
     * @param channelDao 稟議の経路情報を扱うDaoクラス
     * @param rncStatus 稟議の状態
     * @param con コネクション
     * @throws SQLException SQL実行時エラー
     */
    private void ringiModeSet(Rng030ParamModel paramMdl, int rngSid, int userSid,
            RingiDataModel model, String cmd, RngChannelDao channelDao,
            int rncStatus, Connection con) throws SQLException {
        //稟議が完了している場合
        if (model.getRngCompflg() == RngConst.RNG_COMPFLG_COMPLETE) {
            ...
            if (model.getRngStatus() == RngConst.RNG_STATUS_REJECT) {
                ...
                if (!rngReceptionConf(ringiDao, userSid, model.getRngSid())) {
                    paramMdl.setRng030compBtnFlg(0);
                } else if (model.getRngApplicate() == userSid) {
                    paramMdl.setRng030compBtnFlg(1);
                } else if (channelDao.isBeforeApproval(rngSid, userSid)) {
                        paramMdl.setRng030compBtnFlg(1);
                }
            }
         }
    ...
    public boolean appRejBtnConf(Connection con,
        ...
        //稟議が受信されているか判定
        //稟議を受信していなかった場合は操作ができない
        if (!rngReceptionConf(ringiDao, userSid, rngModel.getRngSid())) {
            return false;
        }
    ...
    public boolean compBtnConf(Connection con,
        ...
        //稟議が受信されているか判定
        //稟議を受信していなかった場合は操作ができない
        if (!rngReceptionConf(ringiDao, userSid, rngSid)) {
            return false;
        }
    ...
    public boolean skipBtnConf(Connection con,
        ...
        int rngSid = rngModel.getRngSid();
        RngChannelDao channelDao = new RngChannelDao(con);
        RngChannelModel channelData = channelDao.select(rngSid, rngModel.getRngApplicate());
     ...
     * @return ボタンの実行可能判定
     */
    public boolean copyApprBtnConf(Connection con,
            Rng030ParamModel paramMdl,
            int userSid) throws SQLException {
        RingiDao ringiDao = new RingiDao(con);
        RingiDataModel rngModel = ringiDao.getRingiData(paramMdl.getRngSid(), userSid);
