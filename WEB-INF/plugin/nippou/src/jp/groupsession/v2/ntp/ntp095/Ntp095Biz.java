...
public class Ntp095Biz {
    ...
    public void setInitData(Ntp095ParamModel paramMdl,
        ...
        if (forumSltGp.equals(Ntp095Form.GRP_SID_GRPLIST)) {
            ...
            if (admConf.getNacCrange() == GSConstSchedule.CRANGE_SHARE_ALL) {
                ...
                //所属グループのみ取得
                List<NtpLabelValueModel> allGpBelongList
                    = biz.getGroupLabelForNippou(sessionUsrSid, con, false);
            ...
            //閲覧不可のユーザを取得
            List<Integer> notAccessList = ntpDao.getNotAccessUserList(sessionUsrSid);

            for (int i = 0; i < usList.size(); i++) {
                if (notAccessList.contains(usList.get(i).getUsrSid())) {
                    //閲覧不可のユーザを除外
                    usList.remove(i);
                    i--;
                } else {
                    CmnUsrmInfModel cuiMdl = usList.get(i);
                    labelListAdd.add(new NtpLabelValueModel(cuiMdl, "0"));
                }
            }
        }
    ...
    throws SQLException {
        ...
        for (int i = 0; i < ulist.size(); i++) {
            if (notAccessList.contains(ulist.get(i).getUsrSid())) {
                ulist.remove(i);
                i--;
            }
        }
        ...
        for (CmnUsrmInfModel umodel : ulist) {
            lavelBean = new NtpLabelValueModel(umodel, "0");
            ret.add(lavelBean);
        }
        return ret;
    }
    ...
    public String[] getAddMember(String[] addSelectSid, String[] memberSid,
            int sessionUsrSid, HttpServletRequest req) throws SQLException {
