...
public class FileSearchDao extends AbstractDao {
    ...
    throws SQLException {
        ...
        //対象項目にフォルダがない場合
        if (!StringUtil.isNullZeroString(prmModel.getTargetFolder())
         && !(prmModel.getTargetFolder().equals(String.valueOf(GSConstFile.GET_TARGET_FOLDER)))) {
            sql.addSql("    and");
            sql.addSql("    DIRECTORY.FDR_KBN = 1");

        //対象項目にファイルがない場合
        } else if (!StringUtil.isNullZeroString(prmModel.getTargetFile())
         && !(prmModel.getTargetFile().equals(String.valueOf(GSConstFile.GET_TARGET_FILE)))) {
            sql.addSql("    and");
        ...
        if (prmModel.getPersonalFlg() == GSConstFile.CABINET_KBN_PRIVATE
         ...
         && prmModel.getAconfMdl().getFacUseKbn() == GSConstFile.CABINET_AUTH_USER) {
            sql.addSql("   and ");
            sql.addSql("   exists ");
            sql.addSql("   (select 1");
            sql.addSql("     from (");
            ...
            sql.addSql("     ) as A");
            sql.addSql("     where");
            sql.addSql("       A.USR_SID = DIRECTORY.FCB_OWNER_SID");
            sql.addSql("   )");
            sql.addIntValue(GSConstFile.USER_KBN_GROUP);
            sql.addIntValue(GSConstFile.USER_KBN_USER);
        }
