...
public class Rsv080Biz extends AbstractReserveBiz {
    ...
    public void updateSort(Rsv080ParamModel paramMdl, int changeKbn)
        ...
        int selectedKeyDispNum = Integer.parseInt(selectedKey.substring(20));
        ...
        } else if (selectedKeyDispNum == keyList.length && changeKbn == 1) {
            return;
        }

        //選択された項目の施設グループSID + ソート順
        int moveSid = Integer.parseInt(selectedKey.substring(0, 10));
        int moveSort = Integer.parseInt(selectedKey.substring(10, 20));
        ...
        //ソート順の通りにデータを作成しなおす
        ArrayList<RsvSisDataModel> rsgMdlList = new ArrayList<RsvSisDataModel>();
        for (String allKey : keyList) {
            int intSid = Integer.valueOf(allKey.substring(0, 10));
            int intSort = Integer.valueOf(allKey.substring(20));
            int allKeyDispSort = intSort;

            if (moveSid != intSid) {
                if (allKeyDispSort == target) {
            ...
            RsvSisDataModel rsgMdl = dao.getSisetuData(intSid);
            rsgMdl.setRsdSort(moveSort);
            rsgMdlList.add(rsgMdl);
        }

        int grpSid = paramMdl.getRsv080EditGrpSid();
        dao.delete(grpSid);
        for (RsvSisDataModel rsgMdl:rsgMdlList) {
            dao.insert(rsgMdl);
        }

        //新しいキーを設定
        String newSid = StringUtil.toDecFormat(moveSid, "0000000000");
        String newSort = StringUtil.toDecFormat(target, "0000000000");
