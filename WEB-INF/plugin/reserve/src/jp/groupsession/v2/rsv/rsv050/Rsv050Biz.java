...
public class Rsv050Biz extends AbstractReserveBiz {
    ...
    public void updateSort(Rsv050ParamModel paramMdl, int changeKbn)
        ...
        //ソート順の通りにデータを作成しなおす
        ArrayList<RsvSisGrpModel> rsgMdlList = new ArrayList<RsvSisGrpModel>();
        for (String allKey : keyList) {
            ...
            RsvSisGrpModel rsgMdl = dao.select(intSid);
            rsgMdl.setRsgSort(moveSort);
            rsgMdlList.add(rsgMdl);
        }

        dao.delete();
        for (RsvSisGrpModel rsgMdl:rsgMdlList) {
            dao.insert(rsgMdl);
        }
