...
public class Rsv060Biz extends AbstractReserveBiz {
    ...
    public void setGroupData(Rsv060ParamModel paramMdl) throws SQLException {
        ...
        ArrayList<UsrLabelValueBean> notAdminLabelList = new ArrayList<UsrLabelValueBean>();
        //グループ一覧の場合
        if (grpComboSid == Integer.valueOf(GSConstReserve.GROUP_COMBO_VALUE)) {
            ...
            for (GroupModel bean : allGpList) {
                if (!fullGrepList.contains(String.valueOf("G" + bean.getGroupSid()))) {
                    notAdminLabelList.add(new UsrLabelValueBean(
            ...
            for (int i = 0; i < usList.size(); i++) {
                ...
                UsrLabelValueBean usrLabel =
                        new UsrLabelValueBean(cuiMdl.getUsiSei() + cuiMdl.getUsiMei(),
                        String.valueOf(cuiMdl.getUsrSid()));
                usrLabel.setUsrUkoFlg(cuiMdl.getUsrUkoFlg());
                notAdminLabelList.add(usrLabel);
            }
     ...
     * @throws SQLException SQL実行時例外
     */
    private ArrayList<UsrLabelValueBean> __getGrpUserLabel(String[] left, Connection con)
    throws SQLException {
        ...
        List<CmnUsrmInfModel> ulist
                = userBiz.getUserList(con, (String[]) usrSids.toArray(new String[usrSids.size()]));
