...
public class RsvCommonBiz {
     ...
     * @throws SQLException SQL実行時例外
     */
    public UDate getInitFrDateAuth(Connection con, RsvUserModel uconf) throws SQLException {
        RsvAdmConfDao admConfDao = new RsvAdmConfDao(con);
     ...
     * @throws SQLException SQL実行時例外
     */
    public UDate getInitToDateAuth(Connection con, RsvUserModel uconf) throws SQLException {
        RsvAdmConfDao admConfDao = new RsvAdmConfDao(con);
        RsvAdmConfModel admConf = admConfDao.select();
     ...
     * @throws SQLException SQL実行時例外
     */
    public int getInitPublicAuth(Connection con, RsvUserModel uconf) throws SQLException {
        RsvAdmConfDao admConfDao = new RsvAdmConfDao(con);
        RsvAdmConfModel admConf = admConfDao.select();
    ...
    public boolean isAbleEditLowAuth(Connection con,
        ...
        if (!(isAbleViewLowAuth(
                con, rsyAuid, rsyEuid, rsyPublic, scdRsSid, sessionUsrSid))) {
            auth = false;
        }
    ...
    */
     ...
     * @param rsySid 施設予約SID
     * @param sessionUsrSid セッションユーザSID
     * @return true:編集権限有り false:編集権限なし
     * @throws SQLException SQL実行例外
     */
    public boolean isCanEdit(RequestModel reqMdl,
                                        Connection con,
                                        int rsySid,
        ...
        rsyModel = rsyDao.select(rsySid);

        int auid = rsyModel.getRsyAuid();
        int euid = rsyModel.getRsyEuid();
        int rsyEdit = rsyModel.getRsyEdit();
        ...
        if (isSisGrpAdmin(con, rsySid, sessionUsrSid)) {
            edit = true;
        //その他編集権限の判定
        } else if (isAbleEditLowAuth(con, auid, euid, rsyEdit,
     ...
     * @param rsySid 施設予約SID
     * @param sessionUsrSid セッションユーザSID
     * @return publicFlg true:公開範囲内 false:公開範囲外
     * @throws SQLException SQL実行例外
     */
    public boolean isWithinPubilicRange(Connection con,
            int rsySid,
            int sessionUsrSid) throws SQLException {
        boolean view = false;
        //施設予約情報を取得
        ...
        //その他閲覧権限の判定
                ...
                con, auid, euid, rsyPublic, scdRsSid, sessionUsrSid)) {
            view = true;
        }
