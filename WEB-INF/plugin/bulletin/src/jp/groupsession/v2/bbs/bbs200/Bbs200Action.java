...
/**
 * <br>[機  能] 掲示板 管理者設定 初期値設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 */
public class Bbs200Action extends AbstractBulletinAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Bbs200Action.class);
    ...
    private ActionForward __doInit(
            ActionMapping map, Bbs200Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
                    throws Exception {

        con.setAutoCommit(true);
        Bbs200ParamModel paramMdl = new Bbs200ParamModel();
        paramMdl.setParam(form);
        Bbs200Biz biz = new Bbs200Biz(getRequestModel(req));
    ...
    private void __setKakuninPageParam(
        ...
        cmn999Form.setUrlOK(map.findForward("mine").getPath() + "?CMD=bbs200Entry");
        ...
        cmn999Form.addHiddenParam("cmd", "bbs200Entry");
        cmn999Form.addHiddenParam("bbs200Init", form.getBbs200Init());
        cmn999Form.addHiddenParam("bbs200IniPostTypeKbn", form.getBbs200IniPostTypeKbn());
        cmn999Form.addHiddenParam("bbs200IniPostType", form.getBbs200IniPostType());
    ...
    private ActionForward __doCommit(
        ...
        if (form.getBbs200IniPostType() == GSConstBulletin.BAC_INI_POST_TYPE_KBN_USER) {
            value = gsMsg.getMessage("cmn.set.eachuser");
        } else if (form.getBbs200IniPostType() == GSConstBulletin.BAC_INI_POST_TYPE_KBN_ADM) {
            value = gsMsg.getMessage("cmn.set.the.admin");
            if (form.getBbs200IniPostTypeKbn() == GSConstBulletin.CONTENT_TYPE_TEXT_PLAIN) {
                value = value + " -> " + "テキスト形式";
            } else if (form.getBbs200IniPostTypeKbn() == GSConstBulletin.CONTENT_TYPE_TEXT_HTML) {
                value = value + " -> " + "HTML形式";
            }
        }
