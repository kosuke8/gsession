...
public class Bbs051Action extends AbstractBulletinAction {
    ...
    private ActionForward __doDecision(ActionMapping map,
        ...
        //メイン画面スレッド表示件数
        value = Integer.toString(form.getBbs051threMainCnt()) + "\n";
        //メイン画面の既読投稿表示
        if (form.getBbs051mainChkedDsp() == GSConstBulletin.BBS_CHECKED_DSP) {
            value += "メイン画面の既読投稿表示：表示\n";
        } else if (form.getBbs051mainChkedDsp() == GSConstBulletin.BBS_CHECKED_NOT_DSP) {
            value += "メイン画面の既読投稿表示：非表示\n";
        }
