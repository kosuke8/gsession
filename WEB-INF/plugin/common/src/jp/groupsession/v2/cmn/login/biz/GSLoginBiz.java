...
public class GSLoginBiz implements ILogin {
    ...
    private LoginResultModel __login(LoginModel model, int  mode) throws Exception {
        ...
        if (smodel.getUsrsid() > GSConstUser.USER_RESERV_SID
            ...
            //ログイン停止ユーザはログイン不可
            ActionErrors errors = new ActionErrors();
            ActionMessage msg = new ActionMessage("error.auth.stop.login");
    ...
    public LoginResultModel loginMbl(LoginModel model) throws Exception {
      ...
      //システム予約ユーザはモバイルでのログインは不可
        ...
        try {
            ...
            // 固体識別番号設定
            saveUuidIfAutoSave(smodel.getUsrsid(), uid, con);
    ...
    /**
     *
     * <br>[機  能] 自動登録設定時 固体識別番号、固体識別番号履歴を登録
     * <br>[解  説]
     * <br>[備  考]
     * @param usrSid ユーザSID
     * @param uid 個体識別番号
     ...
     */
    public void saveUuidIfAutoSave(int usrSid, String uid, Connection con) throws SQLException {
        // 固体識別番号設定取得
        ...
        if (infMdl != null) {
            ...
            if (!StringUtil.isNullZeroStringSpace(uid)) {
                ...
                // 固体識別番号でログイン制御 = する & 固体識別番号自動登録 = する
                if (infMdl.getUsiNumCont()   == GSConstUser.UID_CONTROL
                 && infMdl.getUsiNumAutadd() == GSConstUser.UID_AUTO_REG_OK) {
                    ...
                    if (uidDao.updateUid(usrSid, uid) < 1) {
                        // 無ければ登録
