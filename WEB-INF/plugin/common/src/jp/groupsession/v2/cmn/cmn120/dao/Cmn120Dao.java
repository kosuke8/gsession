...
public class Cmn120Dao extends AbstractDao {
     ...
     * @throws SQLException SQL実行例外
     */
    public List<UsrLabelValueBean> selectSmlAccount(
            ArrayList<String> sacSidss) throws SQLException {

        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Connection con = null;
        UsrLabelValueBean smlUsrMdl = null;
        ...
        try {
            ...
            sql.addSql(" order by SAC_NAME, SAC_SID");
            pstmt = con.prepareStatement(sql.toSqlString());
            sql.setParameter(pstmt);
            log__.info(sql.toLogString());
            rs = pstmt.executeQuery();
            while (rs.next()) {
                smlUsrMdl = new UsrLabelValueBean(rs.getString("SAC_NAME"),
                        "sac" + String.valueOf(rs.getInt("SAC_SID")));
                ret.add(smlUsrMdl);
            }
        } catch (SQLException e) {
            throw e;
     ...
     * @throws SQLException SQL実行例外
     */
    public List<UsrLabelValueBean> selectSmlSelAccount(
            ArrayList<String> sacSidss) throws SQLException {
        ...
        try {
            ...
            rs = pstmt.executeQuery();
            while (rs.next()) {
                smlUsrMdl = new UsrLabelValueBean();
     ...
     * @throws SQLException SQL実行例外
     */
    public List<UsrLabelValueBean> selectCirAccount(
            ArrayList<String> sacSidss) throws SQLException {
     ...
     * @throws SQLException SQL実行例外
     */
    public List<UsrLabelValueBean> selectCirSelAccount(
            ArrayList<String> cacSids) throws SQLException {

        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Connection con = null;
        UsrLabelValueBean cirUsrMdl = null;
        ArrayList<UsrLabelValueBean> ret = new ArrayList<UsrLabelValueBean>();
        con = getCon();
        ...
        try {
            ...
            rs = pstmt.executeQuery();
            while (rs.next()) {
                cirUsrMdl = new UsrLabelValueBean(
                rs.getString("CAC_NAME"),
                "cac" + String.valueOf(rs.getInt("CAC_SID")));
                ret.add(cirUsrMdl);
            }
        } catch (SQLException e) {
            throw e;
    ...
    /**
     * <p>回覧板代表アカウント(作成アカウント)の件数を取得する
     * @param cacSidss 除外するアカウントSID
     * @return cnt アカウント件数
     * @throws SQLException SQL実行例外
     */
    public int selectCirAccountCnt(
            ArrayList<String> cacSidss) throws SQLException {
        ...
        try {
            ...
            if (cacSidss != null && !cacSidss.isEmpty()) {
                sql.addSql(" and ");
                sql.addSql("   CAC_SID not in (");
                for (int i = 0; i < cacSidss.size(); i++) {
                    if (i > 0) {
                        sql.addSql(", ");
                    }
                    sql.addSql(cacSidss.get(i));
                }
                sql.addSql(")");
            }
