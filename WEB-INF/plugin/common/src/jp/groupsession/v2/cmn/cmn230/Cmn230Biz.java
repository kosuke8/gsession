...
public class Cmn230Biz {
    ...
    public void __getGroupTabData(Cmn230ParamModel paramModel,
        ...
        if (groupMap != null && !groupMap.isEmpty()) {
            ...
            //グループ選択不可グループを除外
            if (paramModel.getCmn230banGroupSid() != null) {
                List<String> banGroupSids = new ArrayList<String>();
                banGroupSids = Arrays.asList(paramModel.getCmn230banGroupSid());
    ...
    public void __getUserTabData(Cmn230ParamModel paramModel,
        ...
        //利用不可ユーザを除外
        String[] banUserSids = paramModel.getCmn230banUserSid();
        if (banUserSids != null) {
            for (int i = 0; i < banUserSids.length; i++) {
                usrSids.add(new Integer(NullDefault.getInt(banUserSids[i], -1)));
                usrSids2.add(new Integer(NullDefault.getInt(banUserSids[i], -1)));
                usrSids3.add(new Integer(NullDefault.getInt(banUserSids[i], -1)));
            }
        }
        ...
        if (paramModel.getCmn230SearchFlg() == 0) {
            ...
            //検索条件作成
            String keyWord = paramModel.getCmn230SearchStrSv();
            ShainSearchModel searchModel = new ShainSearchModel();
            searchModel.setShainno(keyWord);
            searchModel.setSei(keyWord);
            searchModel.setMei(keyWord);
            searchModel.setSeikn(keyWord);
            searchModel.setMeikn(keyWord);
        ...
        if (usrSids != null && !usrSids.isEmpty()) {
            ...
            if (paramModel.getSelUserSid() != null
                    && !paramModel.getSelUserSid().equals("")) {
                //画面操作ユーザがいる場合
                usrSids.remove(Integer.valueOf(paramModel.getSelUserSid()));
            }
        ...
        if (usrSids2 != null && !usrSids2.isEmpty()) {
            ...
            if (paramModel.getSelUserSid() != null
                    && !paramModel.getSelUserSid().equals("")) {
                //画面操作ユーザがいる場合
                usrSids2.remove(Integer.valueOf(paramModel.getSelUserSid()));
            }
        ...
        if (paramModel.getCmn230SearchFlg() == 0) {
            ...
            if (!StringUtil.isNullZeroString(paramModel.getSelGroupSv())) {
                paramModel.setCmn230groupSid(paramModel.getSelGroupSv());
            } else {
                paramModel.setCmn230groupSid(paramModel.getCmn230dfGpSidStrSv());
            }
        }
