    ...
    public List<ApiNippouDataModel> getReports(List<NtpDataModel> ntpMdlList)
        ...
        if (!ntpMdlList.isEmpty()) {
            ...
            for (NtpDataModel ntpMdl : ntpMdlList) {
                String usid = null;
                usid = String.valueOf(ntpMdl.getUsrSid());
                if (!usidList.contains(usid)) {
                    usidList.add(usid);
                }
                usid = String.valueOf(ntpMdl.getNipEuid());
                if (!usidList.contains(usid)) {
                    usidList.add(usid);
                }
            }

            // 必要なユーザ情報を一括取得(ユーザSIDをキーにしたHashMapにする)
            HashMap<Integer, CmnUsrmInfModel> userMap = new HashMap<Integer, CmnUsrmInfModel>();
            if (usidList.size() > 0) {
                // List → String[] へ変換して引数にする
                List<CmnUsrmInfModel> list
                            = uDao.getUsersInfList((String[]) usidList.toArray(new String[]{}));
                if (list != null) {
                    for (CmnUsrmInfModel mdl : list) {
                        userMap.put(Integer.valueOf(mdl.getUsrSid()), mdl);
                    }
                }
            }
            ...
            for (NtpDataModel ntpMdl : ntpMdlList) {
                ...
                if (userMap.containsKey(usid)) {
                    dataMdl.setUsrMdl(userMap.get(usid));
                }
