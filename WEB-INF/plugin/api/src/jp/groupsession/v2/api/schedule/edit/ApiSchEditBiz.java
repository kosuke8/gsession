        ...
        if (svReserves != null) {
            for (int i = 0; i < svReserves.length; i++) {
                ...
                if (oldMdl != null) {
                    ...
                    yrkParam.setRsyAuid(schMdl.getScdAuid());
                    yrkParam.setRsyAdate(schMdl.getScdAdate());
                }
                ...
                //施設予約と公開区分が異なるので、変換して登録
                int scdPublic = schMdl.getScdPublic();
                //非公開の場合、施設予約の公開区分を予定ありに変換
                if (scdPublic == GSConstSchedule.DSP_NOT_PUBLIC) {
                    scdPublic = GSConstReserve.PUBLIC_KBN_PLANS;
                //予定あり
                } else if (scdPublic == GSConstSchedule.DSP_YOTEIARI) {
                    scdPublic = GSConstReserve.PUBLIC_KBN_PLANS;
                //所属グループのみ
                } else if (scdPublic == GSConstSchedule.DSP_BELONG_GROUP) {
                    scdPublic = GSConstReserve.PUBLIC_KBN_GROUP;
                }
                yrkParam.setRsyPublic(scdPublic);
            ...
            if (scdPublic == GSConstSchedule.DSP_PUBLIC
                ...
                if (!biz.isEditOk(scdMdl, sessionUsrSid, isAdmin, con__, false)) {
                    ...
                    errorFlg = true;
                        ...
                        //同時登録スケジュールに対する編集
                        textSimultaneousEdit = gsMsg.getMessage("schedule.src.33");
                        errorFlg = true;
                }
