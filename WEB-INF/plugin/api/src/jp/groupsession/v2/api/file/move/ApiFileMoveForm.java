...
public class ApiFileMoveForm extends AbstractApiForm {
    ...
    /**
     *
     * <br>[機  能]移動先ディレクトリが自分自身ではないかを確認
     * <br>[解  説]
     * <br>[備  考]
     * @return ディレクトリが自分かどうか(true:自分 false:自分ではない)
     */
    public boolean isOwnDir() {
        //移動先ディレクトリが自分ではないかを確認
        if (fdrParentSid__.equals(fdrSid__)) {
            return true;
        }
        return false;
    }
    ...
    /**
     *
     * <br>[機  能]移動先ディレクトリの親ディレクトリがループしていないか確認
     * <br>[解  説]
     * <br>[備  考]
     * @param con コネクション
     * @param umodel ユーザモデル
     * @throws SQLException SQL実行時例外
     * @return ディレクトリがループしているかどうか(true:している false:していない)
     */
    public boolean isLoopDir(Connection con, BaseUserModel umodel)
            throws SQLException {
        //移動先ディレクトリの親ディレクトリを確認
        FileDao fileDao = new FileDao(con);
        int fdrParentSid = NullDefault.getInt(fdrParentSid__, -1);
        int fdrSid = NullDefault.getInt(fdrSid__, -1);
        FileModel nowFileMdl = fileDao.getFileInf(fdrParentSid, umodel.getUsrsid());
        for (int i = 0; i < GSConstFile.MAX_LEVEL; i++) {
            if (nowFileMdl.getFdrParentSid() == 0) {
                break;
            }
            if (fdrSid == nowFileMdl.getFdrParentSid()) {
                return true;
            }
            nowFileMdl = fileDao.getFileInf(nowFileMdl.getFdrParentSid(), umodel.getUsrsid());
        }
        return false;
    }
