...
function changeDspPermitMode() {
    ...
    if (viewKbn == ENUM_PERM_KBN.SELECT) {
        $('#permissionViewLabel').hide();
        $('#permissionEditRadio').hide();
        $('#permissionViewLabel').parent().prev().attr("rowspan", 1);
        $('#ntp061NanPermitUsrGrp').appendTo($('#permissionViewLabel').parent());
        $('#ntp061NanPermitUsrGrp').show();
        changeDspPermitUsrSelect(["view","edit"]);
        return;
    }
    if (viewKbn == ENUM_PERM_KBN.TANTO) {
        ...
        $('#permissionViewLabel').parent().prev().attr("rowspan", 1);
        $('#permissionEditRadio').hide();
        $('#ntp061NanPermitUsrGrp').hide();
        return;
    }
    if (viewKbn == ENUM_PERM_KBN.ALL) {
        //閲覧権限を制限しない場合、編集権限区分選択を表示
        $('#permissionViewLabel').show();
        $('#permissionViewLabel').parent().prev().attr("rowspan", 2);
        $('#permissionEditRadio').show();
        $('#ntp061NanPermitUsrGrp').appendTo($('input:radio[name=ntp061NanPermitEdit]').parent());
...
権限設定区分の選択で入力要素を切り替える
*/
function changeDspPermitUsrSelect(useKeys) {
    $('#ntp061NanPermitUsrGrp').usrgrpselect({cmd:"dspSelected", key:useKeys});
}
