...
//                }
    ...
    $("#trAddBtnInEdit").live("click", function(){
        ...
        usrInfAreaStr = "<tr class=\"usrInfArea" + row_number + "\" style=\"display:none;\">"
                      ...
                      +  "<span class=\"" + mukouserClass + "\" style=\"font-size:12px;font-weight:bold;\">"
                      +  ntpUsrName
                      +  "</span>"
                      +  "<div>"
...
//        }
    ...
    //コンタクト履歴詳細ボタン
    $(".cont_detail_btn").live("click", function(){
        var paramStr = $(this).attr('id').split("_");
                        ...
                        if (gudata.length > 0) {
                            ...
                            for (u=0; u<gudata.length; u++) {
                                ...
                                goodAddUsrInfstr  += "<table class=\"tl0\" style=\"border-collapse:collapse;\" width=\"100%\">"
                                                  ...
                                                  +        "<span class=\"text_link2 " + mukouserClass + "\">"
    ...
    //コンタクト履歴データ取得
    $(".contDataGetBtn").live("click", function(){
                ...
                if (data != null || data != "") {
                    ...
                    if (data.length > 0) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            contDataStr += "<tr class=\"td_type1\" style=\"font-size:90%;cursor:pointer;\" id=\"tr_" + dataSid + "\">"
                                       + "<td onclick=\"clickSchName(3," + "'" + dataSid + "'" + ");\" width=\"7%\">"
                                       + "<input type=\"checkbox\" name=\"popContact\" value=\"" + data[i].adcSid + "\" onclick=\"clickMulti();\">"
                                       + "<td onclick=\"clickSchName(3," + "'" + dataSid + "'" + ");\" align=\"center\" width=\"10%\" nowrap=\"nowrap\">" + data[i].adcTime + "</td>"
                                       + "<td onclick=\"clickSchName(3," + "'" + dataSid + "'" + ");\" align=\"left\" width=\"205\" nowrap=\"nowrap\">" + data[i].adcTitle + "</td>"
                                       + "<td width=\"20%\" nowrap=\"nowrap\">"
                                       + "<input id=\"" + dataSid + "\" class=\"ntp_shousai_btn cont_detail_btn\" value=\"詳細\" type=\"button\">"
                                       + "</td>"
                                       + "</tr>";
                        }
                    	...
                    	contDataStr += "<tr>"
                        + "<td align=\"center\" width=\"100%\" colspan=\"4\" nowrap=\"nowrap\">"
                        + "<span style=\"font-size:14px;color:#ff0000;font-weight:bold;\">該当するコンタクト履歴がありません。</span>"
                        + "</td>"
                        + "</tr>";
                    }
        ...
        for (i=0; i<cmtdata.length; i++) {
            ...
            if (cmtdata[i].commentUsrJkbn == 9) {
                ...
                commentStr +=  "</td><td id=\"commentDspAreaTable"
                    ...
                    +  "_"
                    +  cmtdata[i].commentSid
                    +  "\" style=\"padding-left: 10px;\" valign=\"top\"><span class=\"" + commUsrUkoClass + "\" style=\"font-size: 12px; color: rgb(51, 51, 51);\"><b>"
                    +  cmtdata[i].commentUserName
...
function setParentAdrPop(selectSid, rowNumber) {
    ...
    if (companySid != null && companySid != "" && companySid != 0 && companySid != -1
        ...
        $('#' + parentId + 'CompNameArea' + rowNumber).html("<div class=\"text_company\">"
                     ...
                     + "<img src=\"../common/images/delete.gif\" class=\"img_bottom\" alt=\"\" width=\"15\"></a></div>");
    }
...
function setParentAnkenPop(selectSid, rowNumber) {
    ...
    //企業・案件情報取得
    setCompanyWithAnken(ankenSid, rowNumber);
...
//企業・顧客履歴取得
function setCompanyWithAnken(ankenSid, trId) {
  $.ajaxSetup({async:false});
  $.post('../nippou/ntp040.do', {"cmd":"getConpanyWithAnken",
                                 "CMD":"getConpanyWithAnken",
                                 "ankenSid":ankenSid},
  function(data) {
    if (data == null || data == "") {
      return;
