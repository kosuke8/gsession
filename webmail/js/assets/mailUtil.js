...
function initEditorForRecordData(type, initAddress, selectData) {
    ...
    setParamValue("wml010tempDirId", ""); // テンポラリディレクトリIDを初期化
...
function getSearchMailFields() {
  return ["selectedMail", "XID", "dirId", "Date","To","ListTo","Cc","Bcc","From","ListFrom","Subject","Attach","Readed","Reply","Forward","EditMail","Label", "Body", "MailSize", "viewMailSize", "LabelId", "LabelName", "fileId", "fileName", "fileSize"];
}
...
function deleteTempFileFromDetail(messageNum, fileId, fileName, type) {
    var deleteParam = "&wmlViewAccount=" + getAccount()
    + "&wml010selectMessageNum=" + messageNum
    + "&wml010delFileId=" + fileId;
    document.getElementsByName("wml010DelAttachParam")[0].value=deleteParam;
    document.getElementsByName("wml010DelAttachFileId")[0].value=fileId;
    document.getElementsByName("wml010DelAttachListKbn")[0].value=type;

    document.getElementById("msg_dialog11").innerHTML = fileName;

    YAHOO.example.container.dialog11.show();
}

//「添付ファイルの削除」ダイアログを閉じる
function closeDelTempFileDialog() {
  YAHOO.example.container.dialog11.cancel();
  document.getElementsByName("wml010DelAttachParam")[0].value="";
}
