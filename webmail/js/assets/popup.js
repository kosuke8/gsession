...
function init() {
  ...
  var delAttachFromDetail = function() {
      var url = "../webmail/wml010.do";
      var deleteParam =  document.getElementsByName("wml010DelAttachParam")[0].value;
      clearErrorMessge();
      var request = YAHOO.util.Connect.asyncRequest('GET', url + "?CMD=fileDeleteFromDetail" + deleteParam, {
          success: function(o) {
              try {
                ...
                if (mailData != null) {
                    ...
                    //削除した添付ファイル情報を除去する
                    var deleteFileId = document.getElementsByName("wml010DelAttachFileId")[0].value;
                    var type = document.getElementsByName("wml010DelAttachListKbn")[0].value;
                    if (type == 1) {
                        $('#searchDtlTempFile' + deleteFileId).hide();
                    } else {
                        $('#dtlTempFile' + deleteFileId).hide();
                    }
                    reloadMailList();
                }
              } catch (e) {
            	  alert(e);
                  clearErrorMessge();
                  closeDelTempFileDialog()
                  setErrorMessge([msglist.failedAttachDelete]);
              }
...
//             { text:msglist.Cancellation, handler:handleCancel }
  ...
  //メール詳細画面からの添付ファイルを削除 ”削除確認"ダイアログ
  YAHOO.example.container.dialog11 = new YAHOO.widget.Dialog("dialog11",
          { width : "500px",
          fixedcenter : true,
          visible : false,
          constraintoviewport : false,
          modal: true,
          buttons : [
                     { text:msglist.delet, handler:delAttachFromDetail, isDefault:true },
                     { text:msglist.Cancellation, handler:handleCancel }
                   ]
    });
