...
//予約送信
    ...
    // テンポラリディレクトリIDを更新
    var tempDirId = "";
    if (YAHOO.example.EditorData.tempDirId) {
        setTempDirId(YAHOO.example.EditorData.tempDirId);
    }
            ...
            YAHOO.example.app.composeToolbar.on('buttonClick', function(ev) {
                if ((ev.button.id == 'tb_send')) {
                    ...
                    if (strUA.indexOf("GsMobileApps") != -1) {
                        ...
                        formValue += formValue+'&wml010tempDirId=' + getTempDirId();
...
function showSendmailConfirmDialog() {
    ...
    var request = YAHOO.util.Connect.asyncRequest('POST', '../webmail/wml012.do?wml012CMD=saveMail&wml012tempDirId=' + getTempDirId(), {
        success: function(o) {
            try {
              var mailData = o.responseText;
              if (mailData != null) {
              ...
              if (mailData != null) {
                  ...
                  if (smailType) {
                      ...
                      if (jQuery.browser.msie) {
                        ...
                        if (!oldIEFlg) {
                            var obj = $('#' + YAHOO.example.app.editor.id).get(0);
                            obj.focus();
                        }
                      }

                      if (tinyMCE.get(YAHOO.example.app.editor.id)) {
                          if (!oldIEFlg) {
                              tinyMCE.get(YAHOO.example.app.editor.id).selection.setContent(
                ...
                setErrorMessge([msglist.failedMailTemplate]);
            }
