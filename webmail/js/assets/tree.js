...
function drawShainList(shainList) {
    ...
    if (shainList) {
        ...
        for (index = 0; index < shainList.length; index++) {
            var mukoclass="";
            if (shainList[index].UKOFLG == 1) {
                mukoclass="mukouser";
            }
            shainListHTML += '<li><span class="' + mukoclass +'"><a href="javascript:setAdrListAddress(\'' + shainList[index].NAME + '\', \'' + shainList[index].MAIL + '\');">'
                                             + shainList[index].NAME + '</span><br><span class="tree_address">' + shainList[index].MAIL
                                             + '</span></a></li>';
            if ((index + 1) / 50 == 0) {
