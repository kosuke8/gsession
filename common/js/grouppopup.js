 ...
 *
 */
function openAllGroup2_Disabled(
        formOj, selBoxName, selGpSid, myGpFlg, submitCmd,
        svName, svName2, plginId,
        selUsrSid, selGrpFlg, admGpFlg, splitFlg, cmdKbn,
        banUserInp, banGpInp, disableGpInp, svDspName, svDspName2) {
    ...
    if (!flagSubWindow || !subWindow || (flagSubWindow && (subWindow.closed || subWindow.top != subWindow.self))) {
        ...
        if (banUserInp) {
            $("[name='" + banUserInp + "']").each(function(){
                qs.push({type:'hidden', name:'cmn230banUserSid', value:this.value});
            });
        }
        if (banGpInp) {
            $("[name='" + banGpInp + "']").each(function(){
                qs.push({type:'hidden', name:'cmn230banGroupSid', value:this.value});
            });
        }
        if (disableGpInp) {
            $("[name='" + disableGpInp + "']").each(function(){
                qs.push({type:'hidden', name:'cmn230disableGroupSid', value:this.value});
            });
        }
        if (!plginId) {
            if (svDspName) {
                qs.push({type:'hidden', name:'cmn230LeftTitleName', value:svDspName});
            }
            if (svDspName2) {
                qs.push({type:'hidden', name:'cmn230RightTitleName', value:svDspName2});
            }
        }
        for(var i = 0; i < qs.length; i++) {
            var ol = qs[i];
