...
function changeGroup(gpSid){
    ...
    document.forms[0].cmn230SearchStrSv.value="";
    document.forms[0].submit();
    return false;
}
...
function setDisableGroup(){
    //選択不可のフォーラムSIDを取得
    $('input:hidden[name=cmn230disableGroupSid]').each(
            function(i, elem) {
                $('#group_branch' + $(elem).val()).removeAttr("onclick");
                $('#group_branch' + $(elem).val()).attr("class", "disabled_link");
    });
}
